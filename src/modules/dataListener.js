...
// SỬA ĐỔI QUAN TRỌNG: Import các module AI cần thiết, đặc biệt là hàm gửi email mới
const { db, firestore } = require('../firebase');

// SỬA ĐỔI: Import hàm gửi cảnh báo mới từ aiModule
const { sendAlertsIfNeeded } = require('../aiModule'); // Dùng hàm gửi email mới

const { learnAndSaveBaseline } = require('../ai/baselineLearner');
...
const { analyzeLongTermTrends, getWeekIdentifier } = require('../ai/longTermAnalyzer'); 
...
async function saveAlert(deviceID, alertData) {
    ...
    try {
        ...
        const alertDoc = {
            ...
            // Lưu ý: Trong code mới, alerts có thể là mảng string, không phải object có key 'message'
...
ref.on('value', async (snapshot) => {
    try {
        ...
        if (physicalAlert) {
            ...
            // Gửi email cảnh báo vật lý
            // LƯU Ý QUAN TRỌNG: Sử dụng hàm mới sendAlertsIfNeeded
            await sendAlertsIfNeeded(deviceID, physicalAlert.alerts);
        ...
        // BỔ SUNG: 3. TÍCH HỢP HỌC BASELINE (Chỉ chạy định kỳ)
        ...
        if (analysis.alerts && analysis.alerts.length > 0) {
            ...
            // LƯU Ý QUAN TRỌNG: Sử dụng hàm mới sendAlertsIfNeeded
            await sendAlertsIfNeeded(deviceID, analysis.alerts);
